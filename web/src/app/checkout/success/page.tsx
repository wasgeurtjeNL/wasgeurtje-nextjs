...
const SuccessPageWrapper = () => {
  ...
  useEffect(() => {
    ...
    if (paymentIntentId && !paymentVerified.current) {
      ...
      // 🚀 NEW OPTIMIZED FLOW: Check existing order first
      checkOrderStatusOptimized(paymentIntentId);
      return;
    }
  }, [searchParams]);
  ...
  // 🚀 NEW: Optimized order status check for instant loading
  const checkOrderStatusOptimized = async (paymentIntentId: string) => {
    try {
      console.log(`🔍 Optimized flow: Checking order status for ${paymentIntentId}`);
      
      // STEP 1: Get PaymentIntent metadata (fastest - has order info if pre-created)
      const piResponse = await fetch(`/api/stripe/payment-status?payment_intent=${paymentIntentId}`);
      const piData = await piResponse.json();
      
      if (piData.metadata?.woocommerce_order_number) {
        // 🚀 FAST PATH: Order was pre-created, show success immediately
        const orderNumber = piData.metadata.woocommerce_order_number;
        const orderId = piData.metadata.woocommerce_order_id;
        
        console.log(`✅ Fast path: Pre-created order found #${orderNumber}`);
        ...
        setOrderDetails({
          ...
          paymentIntentId,
          status: "completed",
          amount: piData.amount / 100,
          customerEmail: piData.metadata.customer_email,
        });
      ...
      // STEP 2: Check if order exists via search (medium speed)
      const searchResponse = await fetch(`/api/woocommerce/orders/search?payment_intent=${paymentIntentId}`);
      const searchData = await searchResponse.json();
      
      if (searchData.success && searchData.orders?.length > 0) {
        // 🔄 MEDIUM PATH: Order exists via webhook, show success
        const order = searchData.orders[0];
        
        console.log(`✅ Medium path: Webhook-created order found #${order.number}`);
      ...
      // STEP 3: Check payment status and create if needed (slow path)
      if (piData.status === 'succeeded') {
        console.log(`⚠️ Slow path: Payment succeeded but no order found, attempting creation`);
        
        // Try to get order data from session storage
        const successDataStr = sessionStorage.getItem("successOrderData");
        if (successDataStr) {
          const successData = JSON.parse(successDataStr);
          await createOrderFromSessionData(successData.orderData, paymentIntentId);
        } else {
          // Last resort: simulate webhook
          await simulateWebhookForOrder(paymentIntentId);
        }
      } else {
        setError(`Payment was not successful. Status: ${piData.status}`);
        setLoading(false);
      }
      
    } catch (error) {
      console.error('Error in optimized order check:', error);
      // Fallback to legacy flow
      verifyPaymentStatus(paymentIntentId);
    }
  };

  const simulateWebhookForOrder = async (paymentIntentId: string) => {
    try {
      console.log(`🎭 Simulating webhook for ${paymentIntentId}`);
      ...
      if (result.success && result.result?.orderId) {
        ...
        setError("Fout bij het aanmaken van bestelling via webhook simulatie");
      }
      
    } catch (error) {
      console.error('Error simulating webhook:', error);
      setError("Fout bij order verwerking");
    } finally {
      setLoading(false);
    }
  };

  const createOrderFromSessionData = async (orderData: any, paymentIntentId: string) => {
    try {
      setOrderDetails({
        paymentIntentId,
      ...
      console.error('Error creating order from session data:', error);
      setError("Fout bij het aanmaken van bestelling");
      setLoading(false);
    }
  };

  // LEGACY: Keep for fallback compatibility
